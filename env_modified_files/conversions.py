    ...
    def step(self, actions):
        ...
        for agent in self.aec_env.agents:
            if "adversary" in agent:
                tempTermDict[agent] = self.aec_env.terminations[agent]
                
        if all(tempTermDict.values()):
            terminations.update((key, True) for key in terminations)
            truncations.update((key, True) for key in truncations)
